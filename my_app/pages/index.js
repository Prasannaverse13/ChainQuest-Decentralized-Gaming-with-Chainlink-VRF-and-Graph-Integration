...
export default function Home() {
  ...
  //This is used to force react to render the page when we want to.
  ...
  const StartGame = async() => {
    try{
      ...
      const randomGameNFTContract = new Contract(
        RANDOM_GAME_NFT_CONTRACT_ADDRESS,abi,signer
      );
      setLoading(true);
   ...
   */
   ...
   const checkIfGameStarted = async() => {
     try{
       ...
       const randomGameNFTContract = new Contract(
         RANDOM_GAME_NFT_CONTRACT_ADDRESS, abi, provider
       );
       const _gameStarted = await randomGameNFTContract.gameStarted();
       const _gameArray = await subgraphQuery(FETCH_CREATED_GAME());
       const _game = _gameArray.games[0];
       let _logs = [];
       //Initializing the logs array and query the graph for current gameID
       if(_gameStarted){
        ...
        <div className={styles.main}>
          <div>
            ...
            <div className={styles.description}>
              its a lottery game where a winner is chosen at random and wins the entire loterry pool
            </div>
            {renderButton()}
